...
%DIF DEL ionic_liquids_submitted.tex   Tue Feb 12 15:11:10 2019
%DIF ADD ionic_liquids.tex             Thu Feb 14 17:03:25 2019
...
Recently, the propensity of the ionic liquid \DIFdelbegin \DIFdel{\ce{[emim][B(CN)_4]} }\DIFdelend \DIFaddbegin \DIFadd{1-ethyl-3-methylimidazolium tetracyanoborate (\ce{[emim][B(CN)_4]}) }\DIFaddend for the physisorption of \ce{CO_2} has been reported, which makes it a potential solvent for carbon capture.
In the present work, molecular dynamics simulations of the solvation of \ce{CO_2} in ionic liquids \ce{[emim][B(CN)_4]} and \DIFdelbegin \DIFdel{\ce{[emim][NTf_2]} }\DIFdelend \DIFaddbegin \DIFadd{1-ethyl-3-methylimidazolium bis (trifluoromethylsulfonyl) imide (\ce{[emim][NTf_2]}) }\DIFaddend are carried out both at infinite dilution and at high concentrations.
A systematic study is performed by comparing several force fields and assessing the efficacy of \DIFdelbegin \DIFdel{simplification measures such as }\DIFdelend \DIFaddbegin \DIFadd{simplifications in the simulations by using }\DIFaddend rigid-body dynamics and pairwise electrostatics.
...
The most commonly used solvents are \DIFdelbegin \DIFdel{volatile organic compounds, specially }\DIFdelend aqueous solutions of \DIFaddbegin \DIFadd{primary and secondary alkanolamines, such as }\DIFaddend monoethanolamine (MEA).
...
\DIFdelbegin \DIFdel{Three }\DIFdelend \DIFaddbegin \DIFadd{Two }\DIFaddend additional assumptions are made here. First, we assume that the gas phase is either pure solute (which is consistent with the ionic-liquid systems \DIFdelbegin \DIFdel{with which we compare our simulations}\DIFdelend \DIFaddbegin \DIFadd{investigated}\DIFaddend ) or can be approximated by an ideal solution. Second, we assume that the system pressure is moderate, so that a truncated virial equation is enough to represent the gas phase. In this case, Eq.~\eqref{eq:fgas_d} becomes
\begin{equation}
\label{eq:equilibrium pressure}
y_s \exp\left( \DIFdelbegin \DIFdel{\frac{B_s P}{R T} }\DIFdelend \DIFaddbegin \DIFadd{\frac{B_s p}{R T} }\DIFaddend \right) \DIFdelbegin \DIFdel{P }\DIFdelend \DIFaddbegin \DIFadd{p }\DIFaddend = c_s\DIFaddbegin \DIFadd{(p) }\DIFaddend R T \exp \left( \DIFdelbegin \DIFdel{\frac{\Delta G_\text{sim}}{R T} }\DIFdelend \DIFaddbegin \DIFadd{\frac{\Delta G_\text{solv}}{R T} }\DIFaddend \right),
\end{equation}
...
\DIFdelbegin \DIFdel{Our third assumption, supported by preliminary simulations, is that $\Delta G_\text{sim}$ does not vary appreciably with pressure, at least over the range of experimental data considered here. In this way, no iterative simulations are required to compute the equilibrium pressure when solving Eq.~}%DIFDELCMD < \eqref{eq:equilibrium pressure} %%%
\DIFdel{for $P$}\DIFdelend \DIFaddbegin \DIFadd{The equilibrium pressure will be the intersection point between the function $\Delta G_{\text{solv}}(p) $, defined by Eq.~}\eqref{eq:equilibrium pressure}\DIFadd{, and $\Delta G_{\text{sim}} (p)$, as obtained in simulations. Note that besides  $\Delta G_{\text{sim}}$, we are taking into account variations of $c_s$ with pressure. These computations are greatly facilitated by the technique of reweighting \mbox{%DIFAUXCMD
\cite{McDonald1967,McDonald1969}}\hspace{0pt}%DIFAUXCMD
, which uses simulation results from different states to improve the statistics and, particularly important to our case, allows to interpolate expectation values of thermodynamic observables along unsampled states. This implies a significant reduction in computational cost. In particular, we use the Multistate Bennett Acceptance Ratio (MBAR) estimator \mbox{%DIFAUXCMD
\cite{Shirts_2008}}\hspace{0pt}%DIFAUXCMD
, which has been successfully used for the purpose of reweighing ~\mbox{%DIFAUXCMD
\cite{Aimoli2014, Nichele2018, Schieber2018}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

\subsection*{Activity Coefficients}

\DIFdelbegin \DIFdel{As will be shown below, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend prediction of activity coefficients requires additional simulations of the solute as a pure liquid phase, whose properties we denote here by a superscript ``$0$''.
...
The only exception was hexane, for which the use of undamped Coulombic interactions (i.e. $\alpha = 0$) resulted in improved chemical potential predictions. \DIFaddbegin \DIFadd{That cutoff distance was also used for truncating the LJ interactions, for which we applied analytical tail corrections \mbox{%DIFAUXCMD
\cite{allen}}\hspace{0pt}%DIFAUXCMD
. In addition, we used the Lorentz-Berthelot mixing rules to calculate the cross LJ interactions, except for the solutes modeled with the OPLS-AA force fields, which employs geometric combining rules.
}\DIFaddend 
...
In the method adopted here \cite{Chodera_2011_2}, simulating an expanded ensemble comprises doing molecular dynamics (MD) at fixed N, \DIFdelbegin \DIFdel{P}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend , T, and $\lambda_\text{vdW}$ amid Monte Carlo (MC) transitions in the value of $\lambda_\text{vdW}$.
...
In this way, 3 iterations were sufficient in all cases for attaining, in the final round, an acceptable uncertainty in $\Delta G_\text{sim}$ estimated via MBAR \cite{Shirts_2008, Klimovich_2015}. \DIFaddbegin \DIFadd{Considering also the MD runs at each $\lambda_{\text{Coul}}$, the simulation time was 110 ns for each system.
}\DIFaddend 

\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In order to obtain minimum-energy conformations, we }\DIFaddend first carried out MD simulations of isolated ion pairs\DIFdelbegin \DIFdel{in order to obtain minimum-energy conformations}\DIFdelend .
...
\caption{Combined distribution functions of the dihedral angles corresponding to atoms C1-N2-C5-C6 and C2-N2-C5-C6 in the cation \ce{[emim]^+} (see Fig.~\ref{fig:atoms_id}), as sampled in \DIFdelbeginFL \DIFdelFL{NPT-MD }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{NpT-MD }\DIFaddendFL simulations of 250 ion pairs using each model for \ce{[emim][B(CN)_4]}.}
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\caption{Comparison of predicted and experimental densities for different models of \ce{[emim][B(CN)_4]} via NPT-MD simulations at $T = 298.0~\text{K}$ and $P = 1.0~\text{atm}$. The experimental density is $1.036~\mathrm{g/cm^3}$ \cite{Doma_ska_2011}.}
%DIFDELCMD < 	\begin{tabular}{ccccc}
%DIFDELCMD < 		\hline
%DIFDELCMD < 		\hline
%DIFDELCMD < 		\ce{[emim][B(CN)_4]} model                        & 
%DIFDELCMD < 		\multicolumn{2}{c}{Flexible ions \& PPPM-Ewald} &
%DIFDELCMD < 		\multicolumn{2}{c}{Rigid ions \& DSF}      \\
%DIFDELCMD < 		&
%DIFDELCMD < 		$\rho_\text{sim}$ (g/cm\textsuperscript{3}) &
%DIFDELCMD < 		error (\%)                                  &
%DIFDELCMD < 		$\rho_\text{sim}$ (g/cm\textsuperscript{3}) &
%DIFDELCMD < 		error (\%)                                  \\
%DIFDELCMD < 		\hline
%DIFDELCMD < 		Koller \textit{et al}. \cite{Koller_2012}    & 1.046 & 1.0 & 1.038 & 0.2 \\
%DIFDELCMD < 		Batista \textit{et al}. \cite{Batista_2015}  & 0.989 & 4.6 & 0.988 & 4.7 \\
%DIFDELCMD < 		Liu \textit{et al}. \cite{Liu_2014}          & 1.001 & 3.4 & 0.991 & 4.4 \\
%DIFDELCMD < 		Weber and Kirchner \cite{Weber_2016}         & 1.015 & 2.1 & 1.022 & 1.4  \\
%DIFDELCMD < 		\hline
%DIFDELCMD < 		\hline
%DIFDELCMD < 		\label{table:props_dsf}
...
\caption{Combined distribution function of the dihedral angles of the contiguous chains C1-S1-N1-S2 and S1-N1-S2-C2 of anion \ce{[NTf_2]^-}, obtained from \DIFdelbeginFL \DIFdelFL{NPT-MD }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{NpT-MD }\DIFaddendFL simulations of 250 ion pairs of \ce{[emim][NTf_2]} at $T = 298.0~\mathrm{K}$ and \DIFdelbeginFL \DIFdelFL{$P = 1.0~\mathrm{bar}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p = 0.1~\mathrm{MPa}$ }\DIFaddendFL using the model of K\"{o}ddermann \textit{et al}. \cite{Koddermann_2007}.}
\label{fig:die_ntf2}
\end{figure}
...
The simulations were carried out at $T = 298.0~\mathrm{K}$ and \DIFdelbegin \DIFdel{$P = 1.0~\mathrm{bar}$}\DIFdelend \DIFaddbegin \DIFadd{$p = 0.1~\mathrm{MPa}$}\DIFaddend .
Table~\ref{table:henry} contains simulated molar densities, partial and total solvation free energies, as well as Henry's law constants ($K_s$) computed via Eq.~\eqref{eq:henry_eq}. \DIFdelbegin \DIFdel{The uncertainties in $K_s$ were calculated by using a standard error propagation formula.
The computed values }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend should be compared to the experimental data reported by Mahurin \textit{et al}. \cite{Mahurin_2010} (\DIFdelbegin \DIFdel{$K_s = 38.9 \pm 0.03 ~\mathrm{atm}$ }\DIFdelend \DIFaddbegin \DIFadd{$K_s = 3.941 \pm 0.003 ~\mathrm{MPa}$ }\DIFaddend for \ce{CO_2} in \ce{[emim][B(CN)_4]}) and by Finotello \textit{et al}. \cite{Finotello_2008} (\DIFdelbegin \DIFdel{$K_s = 39.0 \pm 0.1 ~\mathrm{atm}$ }\DIFdelend \DIFaddbegin \DIFadd{$K_s = 3.95 \pm 0.01 ~\mathrm{MPa}$ }\DIFaddend for \ce{CO_2} in \ce{[emim][NTf_2]}).
\DIFaddbegin \DIFadd{Uncertainties in $K_s$ were calculated using a standard error propagation formula, given by
}\begin{equation}
\DIFadd{\delta K_s = \beta K_s \sqrt{\left[ \delta^2 (\Delta G_{\text{vdW}}) + \delta^2(\Delta G_{\text{Coul}}) \right]},
}\end{equation}
\DIFadd{with uncertainties in free energy calculate using MBAR \mbox{%DIFAUXCMD
\cite{Shirts_2008}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \begin{table*}
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\caption{Results of solvation free energies and Henry's law constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][NTf_2]} at $T = 298.15~\mathrm{K}$ and $P = 1~\mathrm{atm}$. The experimental Henry constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and in \ce{[emim][NTf_2]} are, respectively, $38.9 \pm 0.03 ~\mathrm{atm}$ \cite{Mahurin_2010} and $39.0 \pm 0.1 ~\mathrm{atm}$ \cite{Finotello_2008}.}
%DIFDELCMD < 	\begin{adjustbox}{max width=\textwidth}
%DIFDELCMD < 		\begin{tabular}{ccccccc}  
%DIFDELCMD < 			\hline\hline
%DIFDELCMD < 			Model & $\tilde{\rho}_\text{sim}$ & $\Delta G_\text{vdW}$  & $\Delta G_\text{Coul}$  & $\Delta G_\text{sim}$ & $K_s$ & error \\
%DIFDELCMD < 			& (mol/dm\textsuperscript{3}) & (kcal/mol) & (kcal/mol) &  (kcal/mol) & (atm)  & (\%) \\
%DIFDELCMD < 			\hline
%DIFDELCMD < %			\multicolumn{7}{c}{\ce{[emim][B(CN)_4]}: $K_{s}^{\text{exp}} = 38.9 \pm 0.03 ~\mathrm{atm}$ \cite{Mahurin_2010}} \\
%DIFDELCMD < 			\multicolumn{7}{c}{\ce{[emim][B(CN)_4]}} \\
%DIFDELCMD < 			Koller \textit{et al}. \cite{Koller_2012} & $4.5925$ & $0.45 \pm 0.01$ & $-1.06 \pm 0.02$ & $-0.61 \pm 0.02$ & $40 \pm 1$ & $2.8$ \\
%DIFDELCMD < 			Batista \textit{et al}. \cite{Batista_2015} & $4.3700$ & $0.378 \pm 0.008$ & $-1.24 \pm 0.01$  & $-0.86 \pm 0.01$ & $24.9 \pm 0.5$ & $38.9$ \\
%DIFDELCMD < 			Liu \textit{et al}. \cite{Liu_2014} & $4.3815$ & $0.226 \pm 0.009$ & $-1.11 \pm 0.01$ & $-0.88 \pm 0.01$ & $24.3 \pm 0.5$ & $37.6$  \\
%DIFDELCMD < 			Weber and Kirchner \cite{Weber_2016} & $4.5221$ & $0.478 \pm 0.009$ & $-1.23 \pm 0.02$ & $-0.75 \pm 0.02$ & $31 \pm 1$ & $20.3$  \\
%DIFDELCMD < 			\hline
%DIFDELCMD < %			\multicolumn{7}{c}{\ce{[emim][NTf_2]}: $K_{s}^{\text{exp}} = 39.0 \pm 0.1 ~\mathrm{atm}$ \cite{Finotello_2008} }\\
%DIFDELCMD < 			\multicolumn{7}{c}{\ce{[emim][NTf_2]}} \\
%DIFDELCMD < 			K\"{o}ddermann \textit{et al}. \cite{Koddermann_2007} & $3.8205$ & $0.389 \pm 0.005$ & $-0.901 \pm 0.006$ & $-0.512 \pm 0.008$ & $39.4 \pm 0.5$  & $0.97$  \\
%DIFDELCMD < 			\hline\hline
%DIFDELCMD < 			%DIFDELCMD < \label{table:henry} %%%
%DIFDELCMD < 		\end{tabular}
...
\DIFdelend \DIFaddbegin \begin{table*}
	\centering
	\caption{Results of solvation free energies and Henry's law constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][NTf_2]} at $T = 298.15~\mathrm{K}$ and $p = 0.1~\mathrm{MPa}$. The experimental Henry constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and in \ce{[emim][NTf_2]} are, respectively, $3.941 \pm 0.003 ~\mathrm{MPa}$ \cite{Mahurin_2010} and $3.95 \pm 0.01 ~\mathrm{MPa}$ \cite{Finotello_2008}.}
...
\DIFdelbegin \DIFdel{One can see in Table \ref{table:henry} }\DIFdelend \DIFaddbegin \DIFadd{Table \ref{table:henry} shows }\DIFaddend that the only model able to reproduce the experimental Henry's law constant of \ce{CO_2} in \ce{[emim][B(CN)_4]} with small deviation is that of Koller \textit{et al}. \cite{Koller_2012}.
...
By using TRAVIS \cite{Brehm_2011}, we have obtained SDFs of both cation and anion around a \ce{CO_2} molecule, sampled from MD trajectories of either \ce{CO_2}+\ce{[emim][B(CN)_4]} or \ce{CO_2}+\ce{[emim][NTf_2]} systems simulated at $T = 298.0~\text{K}$, \DIFdelbegin \DIFdel{$P = 1.0~\mathrm{bar}$}\DIFdelend \DIFaddbegin \DIFadd{$p = 0.1~\mathrm{MPa}$}\DIFaddend , and $x_\mathrm{CO_2} = 0.1$.
...
\DIFdelbegin %DIFDELCMD < \begin{table}
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\caption{Thermodynamic conditions, based on the experimental observations of Refs.~\citenum{Makino_2014} and \citenum{Schilderman_2007}, employed in the solvation study at high concentrations of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][NTf_2]}, as well as the results obtained via expanded ensemble simulations.}
%DIFDELCMD < 	\begin{tabular}{ccc}
%DIFDELCMD < 		\hline\hline
%DIFDELCMD < 		IL & \ce{[emim][B(CN)_4]}  & \ce{[emim][NTf_2]} \\
%DIFDELCMD < 		\hline
%DIFDELCMD < %		model & Ref.~\citenum{Koller_2012} & Ref.~\citenum{Koddermann_2007} \\
%DIFDELCMD < 		\multicolumn{3}{c}{Specification} \\
%DIFDELCMD < 		$T$  & $313.15~\mathrm{K}$ & $312.13~\mathrm{K}$ \\
%DIFDELCMD < 		$P$  & $21~\mathrm{atm}$ & $37~\mathrm{atm}$ \\
%DIFDELCMD < 		$x_s$ & $0.352$ & $0.479$ \\
%DIFDELCMD < 		$m_s$ & $2.4~\mathrm{mol/kg}$ & $2.35~\mathrm{mol/kg}$ \\
%DIFDELCMD < 		\hline
%DIFDELCMD < 		\multicolumn{3}{c}{Simulation Result} \\
%DIFDELCMD < 		$c_s$ & $3.04~\mathrm{mol/dm^3}$ & $2.22~\mathrm{mol/dm^3}$ \\ 
%DIFDELCMD < 		$\Delta G_\text{sim}$ & $-0.755~\mathrm{kcal/mol}$ & $-0.542~\mathrm{kcal/mol}$ \\
%DIFDELCMD < 		$P$ & $18.3~\mathrm{atm}$ & $38.4~\mathrm{atm}$ \\
%DIFDELCMD < 		\hline\hline
%DIFDELCMD < 		%DIFDELCMD < \label{table:solv} %%%
%DIFDELCMD < 	\end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}
...
We \DIFdelbegin \DIFdel{carried out expanded ensemble }\DIFdelend \DIFaddbegin \DIFadd{first carried out }\DIFaddend simulations of liquid solutions at the conditions of Table~\ref{table:solv}. The model of Koller \textit{et al}. \cite{Koller_2012} was the one employed for simulating \ce{[emim][B(CN)_4]}. The computed solvation free energies for the systems \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][NTf_2]} were $-0.755 \pm 0.009 ~\mathrm{kcal/mol}$ and $-0.542 \pm 0.005~\mathrm{kcal/mol}$, respectively. \DIFdelbegin \DIFdel{In Fig.~\ref{fig:pressure}, we present our predictions of the equilibrium pressure ($P$) obtained by solving Eq.~}%DIFDELCMD < \eqref{eq:equilibrium pressure}%%%
\DIFdel{, as well as the }\DIFdelend \DIFaddbegin \DIFadd{Using these values as inputs in Eq.~}\eqref{eq:equilibrium pressure}\DIFadd{, we obtained pressure values of $1.854~\mathrm{MPa}$ and $3.893~\mathrm{MPa}$, as reported in Table~\ref{table:solv}.
The }\DIFaddend corresponding gas-phase fugacities of \ce{CO_2} ($f_s^g$) computed by Eq.~\eqref{eq:fgas_d} \DIFdelbegin \DIFdel{.
The dotted lines correspond to the experimental values (i.e. those used to simulate the liquid solutions).
First, the }\DIFdelend \DIFaddbegin \DIFadd{are given in Table~\ref{table:solv}, as well.
The }\DIFaddend results make clear that assuming ideal gas (\DIFdelbegin \DIFdel{$P = f_s^g$}\DIFdelend \DIFaddbegin \DIFadd{$p = f_s^g$}\DIFaddend ) would have been an oversimplification, specially in the case of \ce{[emim][NTf_2]}.
\DIFdelbegin \DIFdel{Second, }\DIFdelend \DIFaddbegin 

\DIFadd{In order to obtain $\Delta G_\text{sim} (p)$ }\DIFaddend and \DIFdelbegin \DIFdel{most importantly, the computed pressures }\DIFdelend \DIFaddbegin \DIFadd{$c_s(p)$, we carried out extra simulations at pressure values  of p = (1.722, 1.823) MPa  for \ce{CO_2}+\ce{[emim][B(CN)_4]} and at p = (4.053, 4.458) MPa for \ce{CO_2}+\ce{[emim][NTf_2]}.
In this case, we considered a wider range of pressure, given the departure from ideal gas behavior mentioned above.
As described in Sec.~\ref{sec:partial_pressure}, $\Delta G_\text{sim}$ and $c_s$ were interpolated to intermediate pressures by reweighting using MBAR \mbox{%DIFAUXCMD
\cite{Shirts_2008}}\hspace{0pt}%DIFAUXCMD
.
In Fig.~\ref{fig:pressure}, we show for both systems the results of $\Delta G_\text{sim}$, together with the curve of $\Delta G_\text{solv}$ defined by Eq.~}\eqref{eq:equilibrium pressure}\DIFadd{.
Although not shown here, the changes in density are negligible: 0.05 \% and 0.07 \% for \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][NTf_2]}, respectively.
In addition, $\Delta G_\text{sim}$ does not vary appreciably with pressure, at least over the range of experimental data considered here. 
This results in small corrections to the equilibrium pressures of Table~\ref{table:solv}: the intersection points are 1.807 MPa and 4.106 MPa, for \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][NTf_2]}, respectively.
These values }\DIFaddend did not depart considerably from the specified values, meaning that our results sustain the experimental observations.
...
\begin{figure}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{pressure_est.pdf}
%DIFDELCMD < \caption{%%%
\DIFdelFL{Prediction of the gas-phase fugacity and partial pressure of \ce{CO_2} for the systems }\DIFdelendFL \DIFaddbeginFL \centering
	\subfloat[ \DIFaddendFL \ce{CO_2}\DIFdelbeginFL \DIFdelFL{-}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{+}\DIFaddendFL \ce{[emim][B(CN)_4]} \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL ]{
		\includegraphics[width=\linewidth]{pressure_koller.pdf}}

	\subfloat[\DIFaddFL{\ce{CO_2}+}\DIFaddendFL \ce{[emim][NTf_2]}\DIFaddbeginFL ]{  
		\includegraphics[width=\linewidth]{pressure_ludwig.pdf}}
	\caption{\DIFaddFL{Solvation free energy $\Delta G_\text{sim}$ (solid line) and $\Delta G_\text{solv}$ of Eq.~}\eqref{eq:equilibrium pressure} \DIFaddFL{(dashed line) as a function of pressure}\DIFaddendFL .}
	\label{fig:pressure}
\end{figure}
...
Due to the availability of experimental data \cite{Yan_2010, Doma_ska_2011}, the solutes chosen for this study are benzene, hexane, cyclohexane, ethanol, and water. \DIFaddbegin \DIFadd{Uncertainties in activity coefficients were calculated using error propagation, as done for $K_s$.
}\DIFaddend 
...
\DIFadd{In Fig.~\ref{fig:deltag_solutes}, we show the simulated solvation free energies versus the experimental results. These were obtained by using $\Delta G^{0}_\text{exp}$ of Table~\ref{table:mu_solutes} and $\gamma^\infty$  from Ref. \mbox{%DIFAUXCMD
\cite{Doma_ska_2011} }\hspace{0pt}%DIFAUXCMD
as inputs in Eq.~}\eqref{eq:gamma}\DIFadd{. Cyclohexane was not considered, given that $\Delta G^{0}_\text{exp}$ is not available.
For benzene, ethanol and water the results from simulations are consistent with the negative experimental values. However, for hexane the simulations predict a favorable solvation, in clear contradiction with the experimental data.
From the individual contributions of $\Delta G_\text{vdW}$ and $\Delta G_\text{Coul}$, we can conclude that the vdW contribution is largely underestimated.
Also, in }\DIFaddend the case of hexane, one can see in Table \ref{table:gamma} that the available experimental values of $\gamma^\infty$ in \ce{[emim][B(CN)_4]} at very close temperatures ($298.15~\text{K}$ and $303~\text{K}$) differ considerably.
...
%DIF > \begin{figure}
%DIF > 	\includegraphics[width=\linewidth]{free_energy_solutes}
%DIF > 	\caption{Profile of $\Delta G_\text{vdW}$ corresponding to the insertion of a \ce{CO_2} molecule in \ce{[emim][B(CN)_4]}, modeled according to the force fields of Koller \textit{et al}. \cite{Koller_2012} ($\bullet$), Batista \textit{et al}. \cite{Batista_2015} ($\blacksquare$), Liu \textit{et al}. \cite{Liu_2014} ($\times$), and Weber and Kirchner \cite{Weber_2016} ($\blacktriangle$).}
%DIF > 	\label{fig:deltag}
%DIF > \end{figure}
\DIFaddbegin 

\begin{figure}
	\includegraphics[width=\linewidth]{delta_g_sim_exp}
	\caption{\DIFaddFL{Simulated free energy $\Delta G_\text{sim}$ versus the experimental result at $T = 298.15~\text{K}$ for the solutes hexane, benzene, ethanol and water in \ce{[emim][B(CN)_4]}.}}
	\label{fig:deltag_solutes}
\end{figure}

\DIFaddend \section{Concluding remarks}
...
A key factor for its success seems to be the reparametrization of the LJ parameters for the boron atom carried out in the original work, which also allows \DIFdelbegin \DIFdel{one }\DIFdelend to accurately reproduce the experimental density of the pure substance.
...
The solvation free energy results \DIFdelbegin \DIFdel{at infinite-dilution condition }\DIFdelend suggest that a more favorable solvation of \ce{CO_2} occurs in \ce{[emim][B(CN)_4]} than in \ce{[emim][NTf_2]}, despite the similarity in the Henry's law constants observed in both cases.
