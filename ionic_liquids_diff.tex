...
%DIF ADD ionic_liquids.tex             Fri Feb 15 19:58:27 2019
\documentclass[3p,twocolumn]{elsarticle}

%DIF 4c4
%DIF < \usepackage[colorlinks,citecolor=blue]{hyperref}
%DIF -------
%\usepackage[colorlinks,citecolor=blue]{hyperref} %DIF > 
%DIF -------
\usepackage{lineno}
\modulolinenumbers[5]
\usepackage{amsmath}
...
\DIFdelbegin \DIFdel{Our third assumption, supported by preliminary simulations, is that $\Delta G_\text{sim}$ does not vary appreciably }\DIFdelend \DIFaddbegin \DIFadd{To determine the equilibrium condition, it is necessary to know how $\Delta G_{\text{sim}}$ and $c_s$ vary }\DIFaddend with pressure, \DIFdelbegin \DIFdel{at least over the range of experimental data considered here.
In this way, no iterative simulations are required to compute the equilibrium pressure when solving Eq.
~}\DIFdelend \DIFaddbegin \DIFadd{so that Eqs.~}\eqref{eq:free_solv} \DIFadd{and }\DIFaddend \eqref{eq:equilibrium pressure} \DIFdelbegin \DIFdel{for $P$}\DIFdelend \DIFaddbegin \DIFadd{can be solved together.
Fortunately, the Multistate Bennett Acceptance Ratio (MBAR) method \mbox{%DIFAUXCMD
\cite{Shirts_2008} }\hspace{0pt}%DIFAUXCMD
allows us to combine simulation samples obtained at discrete thermodynamic states and compute interpolated expectation values along a continuous range of unsampled states.
This technique, known as reweighting \mbox{%DIFAUXCMD
\cite{McDonald1967,McDonald1969}}\hspace{0pt}%DIFAUXCMD
, has been successfully employed in previous work \mbox{%DIFAUXCMD
\cite{Aimoli2014, Nichele2018, Schieber2018}}\hspace{0pt}%DIFAUXCMD
, where one can find more details about the calculation procedure.
Here, we carry out solvation simulations at a few pressures in the vicinity of the experimental value and employ MBAR to determine both functions $\Delta G_{\text{sim}}(p)$ and $c_s(p)$}\DIFaddend .
