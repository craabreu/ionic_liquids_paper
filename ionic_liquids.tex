...
\usepackage[draft,unicode=true,hidelinks,colorlinks,citecolor=blue]{hyperref}
\usepackage{lineno}
\usepackage{amsmath}
\usepackage{ifthen}
\usepackage{commath}
...
Using the software TRAVIS~\cite{Brehm_2011}, for each model we obtain the combined distribution function (CDF) of the dihedral angles corresponding to the torsions in C1-N2-C5-C6 and C2-N2-C5-C6, as sampled in NPT simulations of 250 ion pairs. The CDF, as an alternative of individual histograms, provides a way to  simultaneously analyze the rigidity of the ring. In Fig.~\ref{fig:dihedrals_emim} we present the results of the CDF for each model.

A first inspection of Fig. reveals a similar behavior of the models of [emim]$^{+}$ due to Koller~\textit{et al.}~\cite{Koller_2012} and Liu~\textit{et al.}~\cite{Liu_2014}. This was expected as both were developed based on the AMBER force field. In addition, Figs.(a) and(b) are symmetric, which means that atoms C1-N2-C2 are coplanar. Koller is the most rigid model, with The most likely angles in Fig.~(a) can be found in a narrow region around 90 and 270. Given that these angles correspond to the same configuration in the \textit{united-atom} version, we build all the rigid bodies specifying 90 for the dihedral angle of C5-C5-N2-C1. The other models present a larger flexibility of the ethyl chain, with high probability regions that extend  ina broad of angle values. However, in the case of Liu and Batista, it was possible to the following approach: we consider the most likely angles as obtained by the indificual histogram of the dihedral C5-C5-N2-C1, which were employed to construct two types of rigid bodies. In the case of Liu , 50 of the rigid bodies were build specifying the angle of 90 for the dihedral C6-C5-N2-C1 and -90 of the others. For Batista, we employed the angles 102.6  and -102.6 for buildign both types for rigid bodies in an equal proportion.
