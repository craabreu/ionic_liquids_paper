...
y_s \exp\left( \frac{B_s p}{R T} \right) p = c_s(p) R T \exp \left( \frac{\Delta G_\text{solv}}{R T} \right),
\end{equation}
...
The equilibrium pressure will be the intersection point between the function $\Delta G_{\text{solv}}(p) $, defined by Eq.~\eqref{eq:equilibrium pressure}, and $\Delta G_{\text{sim}} (p)$, as obtained in simulations. Note that besides  $\Delta G_{\text{sim}}$, we are taking into account variations of $c_s$ with pressure. These computations are greatly facilitated by the technique of reweighting \cite{McDonald1967,McDonald1969}, which uses simulation results from different states to improve the statistics and, particularly important to our case, allows to interpolate expectation values of thermodynamic observables along unsampled states. This implies a significant reduction in computational cost. In particular, we use the Multistate Bennett Acceptance Ratio (MBAR) estimator \cite{Shirts_2008}, which has been successfully used for the purpose of reweighing ~\cite{Aimoli2014, Nichele2018, Schieber2018}.
...
\end{equation}
with uncertainties in free energy calculate using MBAR \cite{Shirts_2008}.

\begin{table*}
	\centering
	\caption{Results of solvation free energies and Henry's law constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]} at $T = 298.15~\mathrm{K}$ and $p = 0.1~\mathrm{MPa}$. The experimental Henry constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and in \ce{[emim][Tf_2N]} are, respectively, $3.941 \pm 0.003 ~\mathrm{MPa}$ \cite{Mahurin_2010} and $3.95 \pm 0.01 ~\mathrm{MPa}$ \cite{Finotello_2008}.}
...
By using TRAVIS \cite{Brehm_2011}, we have obtained SDFs of both cation and anion around a \ce{CO_2} molecule, sampled from MD trajectories of either \ce{CO_2}+\ce{[emim][B(CN)_4]} or \ce{CO_2}+\ce{[emim][Tf_2N]} systems simulated at $T = 298.0~\text{K}$, $p = 0.1~\mathrm{MPa}$, and $x_\mathrm{CO_2} = 0.1$.
...
The corresponding gas-phase fugacities of \ce{CO_2} ($f_s^g$) computed by Eq.~\eqref{eq:fgas_d} are given in Table~\ref{table:solv}, as well.
...
In order to obtain $\Delta G_\text{sim} (p)$ and $c_s(p)$, we carried out extra simulations at pressure values  of p = (1.722, 1.823) MPa  for \ce{CO_2}+\ce{[emim][B(CN)_4]} and at p = (4.053, 4.458) MPa for \ce{CO_2}+\ce{[emim][Tf_2N]}.
In this case, we considered a wider range of pressure, given the departure from ideal gas behavior mentioned above.
As described in Sec.~\ref{sec:partial_pressure}, $\Delta G_\text{sim}$ and $c_s$ were interpolated to intermediate pressures by reweighting using MBAR \cite{Shirts_2008}.
In Fig.~\ref{fig:pressure}, we show for both systems the results of $\Delta G_\text{sim}$, together with the curve of $\Delta G_\text{solv}$ defined by Eq.~\eqref{eq:equilibrium pressure}.
Although not shown here, the changes in density are negligible: 0.05 \% and 0.07 \% for \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][Tf_2N]}, respectively.
...
This results in small corrections to the equilibrium pressures of Table~\ref{table:solv}: the intersection points are 1.807 MPa and 4.106 MPa, for \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][Tf_2N]}, respectively.
...
\begin{figure}
	\centering
	\subfloat[ \ce{CO_2}+\ce{[emim][B(CN)_4]} ]{
		\includegraphics[width=\linewidth]{pressure_koller.pdf}}
	
	\subfloat[\ce{CO_2}+\ce{[emim][Tf_2N]}]{  
...
Due to the availability of experimental data \cite{Yan_2010, Doma_ska_2011}, the solutes chosen for this study are benzene, hexane, cyclohexane, ethanol, and water. Uncertainties in activity coefficients were calculated using error propagation, as done for $K_s$.
...
\begin{figure}
	...
	\caption{Simulated free energy $\Delta G_\text{sim}$ versus the experimental result at $T = 298.15~\text{K}$ for the solutes hexane, benzene, ethanol and water in \ce{[emim][B(CN)_4]}.}
