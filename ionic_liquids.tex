...
For this, we compare results obtained with a standard methodology to those obtained with a simplified one.
...
These angles will be referred to as $\psi$ and $\chi$, respectively.
In Fig.~\ref{fig:dihedrals_emim}, we show the CDFs obtained for all force fields employed here for \ce{[emim][B(CN)_4]}.
The CDFs were generated from NPT-MD simulations of 250 ion pairs at $T = 298.0~\mathrm{K}$ and $P = 1.0~\mathrm{bar}$ using our standard setup.
Initial inspection reveals that, in all cases, $\psi$ and $\chi$ populate regions of the diagrams around the two straight lines given by
\begin{equation}
\label{eq:dihedral angles}
\psi = \begin{cases}
\chi + 180^\circ & \text{if} \quad \chi \leq 180^\circ \\
\chi - 180^\circ & \text{if} \quad \chi > 180^\circ
\end{cases},
\end{equation}
...
One can note in Fig.~\ref{fig:dihedrals_emim} that the CDF peaks (i.e. the most likely dihedral angle combinations) lie either precisely above these lines or very close to them.
...
Therefore, for specifying the torsion of bond N2-C5 it is enough to set a value for $\chi$ and then compute $\psi$ by means of Eq.~\eqref{eq:dihedral angles}.
...
In both diagrams, the most likely angle combinations can be found in narrow regions around $\chi = 90^\circ$ and $\chi = 270^\circ$.
...
Hence, we decided to build a single rigid-body structure with $\chi = 90^\circ$ for the united-atom version of \ce{[emim]^+}, which is the model of Koller \textit{et al}. \cite{Koller_2012}.
In the case of the all-atom model of Liu \textit{et al}. \cite{Liu_2014}, we considered a system composed by equal amounts of two rigid-body structures with $\chi = 90^\circ$ and $\chi = 270^\circ$.
For the Batista \textit{et al}. \cite{Batista_2015} model, the regions of likely angle combinations comprise convoluted peaks.
By means of weighted averages, we decided to set $\chi = 77.4^\circ$ and $\chi = 282.6^\circ$ so as to build two types of rigid bodies, also considered in equal amounts.
Finally, given the wide ranges of likely angle combinations in the case of the Weber and Kirchner model \cite{Weber_2016}, as observed in Fig.~\ref{fig:dihedrals_emim}(d), we defined four types of rigid bodies in this case to fulfill the system in different proportions.
The $\chi$-angle values for these types are $0^\circ$, $30^\circ$, $180^\circ$, and $330^\circ$, while their fractions in the system are 25\%, 30\%, 15\%, and 30\%, respectively.
