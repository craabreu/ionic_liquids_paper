...
\usepackage[position=top,captionskip=-12pt]{subfig}
\usepackage{graphicx}
...
\begin{equation}
\Delta G_\text{sim} \equiv R T \ln \frac{\Xi (N_s,N_\text{\tiny IL},P,T,\lambda = 1)}{\Xi (N_s,N_\text{\tiny IL},P,T,\lambda = 0)}, 
\end{equation}
...
\Delta G_{\text{solv}} \equiv R \ln \left( \frac{P_s}{c_s R T} \right),
\end{equation}
...
\end{equation}
where $\tilde{\rho}_\text{sim}$ is the simulated molar density of the solution. Assuming that this solution is very dilute, one can use the molar density of pure solvent ($\tilde{\rho}_\text{\tiny IL}$) instead.
...
\begin{equation}
\label{eq:fgas_d}
f^{g}_s = \exp \left( \frac{\Delta G_\text{sim}}{R T} \right) +  c_s R T. 
\end{equation}
...
y_s \exp\left( \frac{B_s P}{R T} \right) P = \exp \left( \frac{\Delta G_\text{sim}}{R T} \right) +  c_s R T,
\end{equation}
...
A study of conformations was also carried out for \ce{[emim][NTf_2]} using the model of K\"{o}ddermann \textit{et al}. \cite{K_ddermann_2007}.
...
\caption{Combined distribution function of the dihedral angles of the contiguous chains C1-S1-N1-S2 and S1-N1-S2-C2 of anion \ce{[NTf_2]^-}, obtained from NPT-MD simulations of 250 ion pairs of \ce{[emim][NTf_2]} using the model of K\"{o}ddermann \textit{et al}. \cite{K_ddermann_2007}.}
\label{fig:die_ntf2}
\end{figure}
...
\begin{table*}
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{ccccccc}  
			\hline\hline
			Model & $\tilde{\rho}_\text{sim}$ & $\Delta G_\text{\tiny vdW}$  & $\Delta G_\text{\tiny Coul}$  & $\Delta G_\text{sim}$ & $K_s$ & error \\
...
Is it worth remarking that the result of $\Delta G_\text{sim}$ obtained with the model of Liu \textit{et al}. \cite{Liu_2014} is statistically identical to a value reported by the same authors in Ref.~\citenum{Liu_2014_1}, where they applied the Bennett Acceptance Ratio (BAR) method \cite{Bennett_1976} to independent samples obtained with different values of $\lambda_\text{\tiny vdW}$ and $\lambda_\text{\tiny Coul}$.
...
These models fail in part because of the small values they predict for $\Delta G_\text{\tiny vdW}$.
In order to analyze the model distinctions in more detail, in Fig.~\ref{fig:deltag} we plot free energy profiles with respect to the coupling parameter $\lambda_\text{\tiny vdW}$, as obtained from the simulations with all \ce{[emim][B(CN)_4]} models.
...
The Liu \textit{et al}. \cite{Liu_2014} model predicts the smallest value of $\Delta G_\text{\tiny vdW}$ in spite of exhibiting only the second smallest free-energy barrier.
This occurs because the attractive van der Waals interactions compensate the cavity formation more favorably than in the case of Batista \textit{et al}. \cite{Batista_2015}, as one can observe in Fig.~\ref{fig:deltag} as $\lambda_\text{\tiny vdW}$ approaches unity.
Nevertheless, these distinctions end up being canceled out when the charges of the \ce{CO_2} molecule are introduced, since $\Delta G_\text{\tiny vdW}$ and $\Delta G_\text{\tiny Coul}$ sum up to very similar $\Delta G_\text{sim}$ results for both models.

\begin{figure}
\includegraphics[width=\linewidth]{free_energy_paper}
\caption{Profile of $\Delta G_\text{\tiny vdW}$ corresponding to the insertion of a \ce{CO_2} molecule in \ce{[emim][B(CN)_4]}, modeled according to the force fields of Koller \textit{et al}. \cite{Koller_2012} ($\bullet$), Batista \textit{et al}. \cite{Batista_2015} ($\blacksquare$), Liu \textit{et al}. \cite{Liu_2014} ($\times$), and Weber and Kirchner \cite{Weber_2016} ($\blacktriangle$).}
\label{fig:deltag}
\end{figure}
...
Also, the first solvation shell due to \ce{[B(CN)_4^-} occurs at a slightly shorter distance than for \ce{[NTf_2]^-}, as one sees from Fig.~\ref{fig:sdf_ions}(c).
...
\begin{table}
	...
	\begin{tabular}{ c  c  c  }  
		...
		\hline
		model &  Ref.~\citenum{Koller_2012} & Ref.~\citenum{K_ddermann_2007} \\
		T  & 313.15 K & 312.13 K \\
		P  & 21 atm & 37 atm \\
		$x_s$ & 0.352 & 0.479 \\
		$m_s$ & 2.4 mol/kg & 2.35 mol/kg \\
		\hline\hline
		\label{table:solv} 
	\end{tabular}
\end{table}
...
The solvation free energies obtained for \ce{CO_2}-\ce{[emim][B(CN)_4]} and \ce{CO_2}-\ce{[emim][NTf_2]} were $-0.755 \pm 0.01 ~\mathrm{kcal/mol}$ and $-0.542 \pm 0.005$, respectively.
...
\begin{table*}
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{ccccccccc}
			\hline\hline
			Solute & $\rho_\text{sim}$ & error & $\Delta G_\text{\tiny vdW}$  & $\Delta G_\text{\tiny Coul}$  & $\Delta G^0_\text{sim}$ & $\Delta G^0_\text{exp}$   & error \\
...
\begin{table*}
	...
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{cccccccc}  
			\hline\hline
			Solute & $\Delta G_\text{\tiny vdW}$  & $\Delta G_\text{\tiny Coul}$  & $\Delta G_\text{sim}$  & $\gamma^\infty_\text{sim}$ & $\gamma^\infty_\text{exp}$ \cite{Doma_ska_2011} & $\gamma^\infty_\text{exp}$ \cite{Yan_2010} \\
