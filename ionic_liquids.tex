...
The differences in \ce{CO_2} molar fractions for this ILs exemplifies the observations made by Carvalho \textit{et al.} \cite{Carvalho_2016}, mentioned at the introduction of this paper.
...
In Fig.~\ref{fig:pressure} we show for each system our predictions of the gas-phase pressure ($P_s$), as described in Sec.~\ref{sec:partial_pressure}.
...
One of the goals of this paper was to compare available models of \ce{[emim][B(CN)_4]} in terms of their capability to reproduce the experimental Henry constant of \ce{CO_2}.
We have found that only the model of Koller \textit{et al.} \cite{Koller_2012} achieves this goal.
A key factor for this success seems to be the reparametrization of the LJ parameters for the boron atom, which is only performed in the work of Koller \textit{et al.}  \cite{Koller_2012} in order to reproduce the experimental density of the pure substance.

In the case of \ce{[emim][B(CN)_4]}, we were able to 

to introduce computationally efficient strategies, such as the use of rigid bodies and the DSF method for evaluating the Co the simulations and perform them in a more computationally efficient way: the ILs were modelled as rigid bodies and using the DSF method to calculate the Coulombic interactions.
This required some preliminary simulations in order to analyze the most probable conformations of the ions.
This was carried out in a simple way and with very good results.

Using the expanded ensemble method, we reproduced the solvation free energy for both systems of interest:
\ce{CO_2}-\ce{[emim][B(CN)_4]} and \ce{CO_2}-\ce{[emim][NTf_2]} and at different thermodyanamic conditions. 
At infinite-dilution those solvents performs in the solvation in \ce{[emim][B(CN)_4]} only slightly more favorable.
This picture completely changed when we analyzed the systems at higher concentrations: the pressure required to dissolved a certain mass of \ce{CO_2} is considerable smaller that for \ce{[emim][NTf_2]}.
