...
After a short run with $\Delta t$ = $0.5~/text{fs}$, we carried out a ``quenching'' simulation with $\Delta t$ = $1 ~/text{fs}$, in which the temperature is gradually reduced until reaching a value close to $0~/text{K}$.
...
In Fig.~\ref{fig:dihedrals_emim}(d), which corresponds to Weber and Kirchner \cite{Weber_2016}, one sees that the angle As this is the most flexible model, we defined four types of rigid bodies: 150$^{\circ}$, 180$^{\circ}$ and 210$^{\circ}$ were employed to build 30~\%, 25~\% and 30~\% of them, respectively; and 0$^{\circ}$ for the remaining 15~\%.
...
Table \ref{table:henry} also includes the results for [emim][Tf\textsubscript{2}N] as well as the experimental $K_s$ reported by Mahurin \textit{et al.} \cite{Mahurin_2010} and Finotello \textit{et al.} \cite{Finotello_2008}.
...
The spatial distribution functions (SDFs) \cite{Svishchev_1993} shown in Figs.~\ref{fig:sdf_ions} confirm these results.
...
For that, we carried out NPT-MD simulations of the systems \ce{CO_2}-\ce{[emim][B(CN)_4]} and \ce{CO_2}-[emim][Tf\textsubscript{2}N] with a solute molar fraction of 0.1.
The isovalues were visually determined so as to observe the first solvation shell.
From Fig.~\ref{fig:sdf_ions}(b), which correspond to the ions \ce{[emim]^+} and \ce{[NTf_2]^-}, it can be seen that the region with a low probability of finding an ion is larger than for \ce{[emim][B(CN)_4]}.
Also, the first solvation shell due to [B(CN)\textsubscript{4}]$^{-}$ occurs at a sligthly shorter distance than for \ce{[NTf_2]^-}, as one can note from Fig.
\begin{figure}[ht]
\centering
\subfloat[]{%
...
Fig. (c) corresponds to the SDFs of the anions, only.}
\label{fig:sdf_ions}
\end{figure}
...
Having selected the model of \ce{[emim][B(CN)_4]} that best reproduces the experimental $K_s$ of \ce{CO_2}, we carried out the calculation of infinite-dilution activity coefficients ($\gamma^{\, \infty}$) of benzene, hexane, cyclohexane, ethanol and water in \ce{[emim][B(CN)_4]}.
These solutes were modeled as described in Sec.~\ref{sec:force_field_sol}.
