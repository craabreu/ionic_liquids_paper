...
\begin{equation}
\Delta G_{\text{solv}} =  k_b T \ln \left( \frac{P_s}{c^l_s R T} \right),
\end{equation}
where $R=N_Ak_b$ is the gas constant ($N_A$ is the Avogadro number) and, for a system at phase equilibrium, $P_s$ is the partial pressure of the solute in the gas phase and $c^l_s$ is its molar concentration in the liquid phase. For obtaining the mentioned relation, the authors derived an expression for the chemical potential of the solute in the liquid phase ($\mu^l_s$) as a function of $\Delta G_\text{sim}$, which can be written as
\begin{equation}
\mu^l_s = \mu^{ig}_s + \Delta G_\text{sim} - k_bT \ln \left( \frac{V^{\ast}}{V_1}\right) - k_bT \ln \left( \frac{P_s}{c_s^{\ast} k_b T} \right),
\end{equation}
where $\mu^{ig}_s$ is the chemical potential of the solute in an ideal gas at temperature $T$ and solute partial pressure $P_s$, $V^{\ast}$ and $V_1$ are the mean volumes of liquid solutions specified by ($N_s-1$,$N_\text{\tiny IL}$,P,T) and ($N_s$,$N_\text{\tiny IL}$,P,T), respectively, while $c_s^{\ast} = N_s/V_1$ is the number concentration of the solute in the latter.
...
Finally, by treating the gas phase as an ideal gas and considering a system at phase equilibrium ($\mu^l_s = \mu^{ig}_s$), Shirts \textit{et al}. \cite{Shirts_2003} then found out that
\begin{equation}
\begin{split}
\label{eq:free_solv}
...
If both $P_s$ and $c_s^{\ast}$ are small, we can resort to Eq.~\eqref{eq:free_solv} to predict Henry's law constant $\text{K}_s$, which is defined as \cite{Prausnitz}
\begin{equation}
\text{K}_s =\frac{P_s}{x_s}.
\end{equation}

Therefore, by simply expressing Eq.~\eqref{eq:free_solv} in terms of the molar fraction $x_s$ and assuming that the molar density of the solution does not perceivably differ from than of the solvent ($\tilde{\rho}_\text{\tiny solution} \approx \tilde{\rho}_\text{\tiny IL}$), we obtain that
\begin{equation}
\label{eq:henry_eq}
K_s = \tilde{\rho}_\text{\tiny IL} R \, T \exp(\beta \, \Delta G_\text{sim}),
\end{equation}
where $R$ is the gas constant and $\beta = (k_b T)^{-1}$.
...
This equation, together with the fact that $f^{g}_s$ = $P_s \phi_s$, are used to evaluate the phase equilibrium equation $\mu^{l}_s$ = $\mu^{g}_s$, which yields
\begin{equation}
\label{eq:fgas_d}
