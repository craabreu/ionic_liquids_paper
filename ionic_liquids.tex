...
In order to analyze the model distinctions in more detail, in Fig.~\ref{fig:deltag} we plot the profiles of $\Delta G_\text{\tiny vdW}$ as a function of the coupling parameter $\lambda_\text{\tiny vdW}$.

If we look at the $\Delta G_\text{\tiny vdW}$ profiles in Fig.~\ref{fig:deltag}, we observe that for the models of Batista \textit{et al}. \cite{Batista_2015} and Liu \textit{et al}.~\cite{Liu_2014} the maximum occurs at lower values of energy, suggesting that these models underestimate the atomic interactions, which may also explain their failure to reproduce the experimental density.
