...
\title{A molecular dynamics study of the solvation of carbon dioxide and other compounds in the ionic liquids \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]}}
...
Recently, the propensity of the ionic liquid 1-ethyl-3-methylimidazolium tetracyanoborate (\ce{[emim][B(CN)_4]}) for the physisorption of \ce{CO_2} has been reported, which makes it a potential solvent for carbon capture.
In the present work, molecular dynamics simulations of the solvation of \ce{CO_2} in ionic liquids \ce{[emim][B(CN)_4]} and 1-ethyl-3-methylimidazolium bis (trifluoromethylsulfonyl) imide (\ce{[emim][Tf_2N]}) are carried out both at infinite dilution and at high concentrations.
...
This is true for the popular ILs \ce{[bmim][Tf_2N]}, \ce{[bmim][PF_6]}, \ce{[bmim][BF_4]}, as well as for those synthesized from the fluorination of the cation or the anion, such as \ce{[(C_2H_2F_2)mim][Tf_2N]} and \ce{[hmim][pFAP]}.
...
For comparison, we analyzed the solvation of \ce{CO_2} not only in \ce{[emim][B(CN)_4]}, but also in \ce{[emim][Tf_2N]}, both at infinite dilution and at high concentrations.
...
Two additional assumptions are made here. First, we assume that the gas phase is either pure solute (which is consistent with the ionic-liquid systems investigated) or can be approximated by an ideal solution. Second, we assume that the system pressure is moderate, so that a truncated virial equation is enough to represent the gas phase. In this case, Eq.~\eqref{eq:fgas_d} becomes
\begin{equation}
\label{eq:equilibrium pressure}
y_s \exp\left( \frac{B_s p}{R T} \right) p = c_s(p) R T \exp \left( \frac{\Delta G_\text{sim}}{R T} \right),
\end{equation}
...
The equilibrium pressure will be given by the value for which the function $\Delta G_{\text{sim}}(p) $, defined by Eq.~\eqref{eq:equilibrium pressure}, intersects $\Delta G_{\text{sim}} (p)$, as obtained in simulations. Note that besides  $\Delta G_{\text{sim}}$, we are also considering  variations of $c_s$ with pressure. These computations are greatly facilitated by the technique of reweighting \cite{McDonald1967,McDonald1969}, which uses simulation results from different states to improve the statistics and, particularly important to our case, allows to interpolate expectation values of thermodynamic observables along unsampled states. This implies a significant reduction in computational cost. In particular, we use the Multistate Bennett Acceptance Ratio (MBAR) estimator \cite{Shirts_2008}, which has been successfully used for the purpose of reweighing ~\cite{Aimoli2014, Nichele2018, Schieber2018}.
...
In the case of ionic liquid \ce{[emim][Tf_2N]}, we use the model of K\"{o}ddermann \textit{et al}. \cite{Koddermann_2007}, which was also derived by refining parameters of the force field by P{\'{a}}dua and Canongia Lopes \cite{Canongia_Lopes_2006}.
...
Kerl \textit{et al}. \cite{Kerl__2017} recently employed this model to accurately predict Henry constants of \ce{CO_2} in \ce{[emim][Tf_2N]} at several temperatures.
...
In this way, 3 iterations were sufficient in all cases for attaining, in the final round, an acceptable uncertainty in $\Delta G_\text{sim}$ estimated via MBAR \cite{Shirts_2008, Klimovich_2015}. Considering also the MD runs at each $\lambda_{\text{Coul}}$, the simulation time was 110 ns for each system.
...
For building the rigid bodies, we carry out a frequency analysis of some key dihedrals present in ions \ce{[emim]^+} and \ce{[Tf_2N]^-}, whose flexibility could lead to significant conformational changes.
...
As a guide to the discussions in this section, in Fig~\ref{fig:atoms_id} we introduce a nomenclature for all atoms present in the ions \ce{[emim]^+}, \ce{[B(CN)_4]^-}, and \ce{[Tf_2N]^-}.
...
\caption{Adopted nomenclature for individual atoms in the cation 1-ethyl-3-methylimidazolium (\ce{[emim]^+}) and anions tetracyano borate (\ce{[B(CN)_4]^-}) and bis(trifluoromethylsulfonyl)amide (\ce{[Tf_2N]^-}).}
\label{fig:atoms_id}
\end{figure}
...
A study of conformations was also carried out for \ce{[emim][Tf_2N]} using the model of K\"{o}ddermann \textit{et al}. \cite{Koddermann_2007}.
...
However, the most likely value of $\psi_1$ is shifted to $0^{\circ}$ in the case of \ce{[emim][Tf_2N]}.
...
With regard to the anion \ce{[Tf_2N]^-}, Deetlefs \textit{et al}. \cite{Deetlefs_2006} experimentally determined the conformer population of the liquid-phase and observed that \ce{[Tf_2N]^-} is mostly found in its \textit{trans} conformation.
In Fig.~\ref{fig:die_ntf2}, we present the CDF obtained for the contiguous sequences C1-S1-N1-S2 and S1-N1-S2-C2 in \ce{[Tf_2N]^-}, which qualitatively confirms the observation of Deetlefs \textit{et al}. \cite{Deetlefs_2006}.
This CDF also makes clear that \ce{[Tf_2N]^-} exhibits a considerable degree of flexibility.
Due to this observed behaviors of both ions, all simulations of \ce{[emim][Tf_2N]} in the present work are performed considering flexible cations and anions, as well as DSF electrostatics.
...
\caption{Combined distribution function of the dihedral angles of the contiguous chains C1-S1-N1-S2 and S1-N1-S2-C2 of anion \ce{[Tf_2N]^-}, obtained from NpT-MD simulations of 250 ion pairs of \ce{[emim][Tf_2N]} at $T = 298.0~\mathrm{K}$ and $p = 1.0~\mathrm{bar}$ using the model of K\"{o}ddermann \textit{et al}. \cite{Koddermann_2007}.}
\label{fig:die_ntf2}
\end{figure}

\subsection{Dilute Solutions of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]}}
\label{sec:henry_results}

In Table \ref{table:henry}, we present results from expanded ensemble simulations of a single \ce{CO_2} molecule dissolved in 250 ion pairs of either \ce{[emim][B(CN)_4]} or \ce{[emim][Tf_2N]} under periodic boundary conditions.
The simulations were carried out at $T = 298.0~\mathrm{K}$ and $p = 1.0~\mathrm{bar}$.
Table~\ref{table:henry} contains simulated molar densities, partial and total solvation free energies, as well as Henry's law constants ($K_s$) computed via Eq.~\eqref{eq:henry_eq}. The computed values should be compared to the experimental data reported by Mahurin \textit{et al}. \cite{Mahurin_2010} ($K_s = 38.9 \pm 0.03 ~\mathrm{atm}$ for \ce{CO_2} in \ce{[emim][B(CN)_4]}) and by Finotello \textit{et al}. \cite{Finotello_2008} ($K_s = 39.0 \pm 0.1 ~\mathrm{atm}$ for \ce{CO_2} in \ce{[emim][Tf_2N]}).
...
\end{equation}
which are a function of the uncertainties in free energy, which are calculate using MBAR \cite{Shirts_2008}.

\begin{table*}
	\centering
	\caption{Results of solvation free energies and Henry's law constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]} at $T = 298.15~\mathrm{K}$ and $p = 1~\mathrm{atm}$. The experimental Henry constants of \ce{CO_2} in \ce{[emim][B(CN)_4]} and in \ce{[emim][Tf_2N]} are, respectively, $38.9 \pm 0.03 ~\mathrm{atm}$ \cite{Mahurin_2010} and $39.0 \pm 0.1 ~\mathrm{atm}$ \cite{Finotello_2008}.}
...
%			\multicolumn{7}{c}{\ce{[emim][B(CN)_4]}: $K_{s}^{\text{exp}} = 38.9 \pm 0.03 ~\mathrm{atm}$ \cite{Mahurin_2010}} \\
			...
			\hline
%			\multicolumn{7}{c}{\ce{[emim][Tf_2N]}: $K_{s}^{\text{exp}} = 39.0 \pm 0.1 ~\mathrm{atm}$ \cite{Finotello_2008} }\\
			\multicolumn{7}{c}{\ce{[emim][Tf_2N]}} \\
...
Moreover, the $\Delta G_\text{sim}$ value we computed for \ce{[emim][Tf_2N]} is very close to the one obtained by Kerl \textit{et al}. \cite{Kerl__2017} ($-0.534 ~\mathrm{kcal/mol}$, with no reported uncertainty) using the Free-Energy Perturbation (FEP) method \cite{Zwanzig_1954}.
%These consistencies have served for validating the methodological approaches of this work.

Table \ref{table:henry} shows that the only model able to reproduce the experimental Henry's law constant of \ce{CO_2} in \ce{[emim][B(CN)_4]} with small deviation is that of Koller \textit{et al}. \cite{Koller_2012}.
...
Such as the model of Koller \textit{et al}. \cite{Koller_2012} does in the case of \ce{[emim][B(CN)_4]}, the one of K\"{o}ddermann \textit{et al}. \cite{Koddermann_2007} accurately predicts the Henry's law constant of \ce{CO_2} in \ce{[emim][Tf_2N]}.
A comparison between the values of $\Delta G_\text{sim}$ obtained from these models, shown in Table \ref{table:henry}, suggests that the solvation of \ce{CO_2} takes place more favorably in \ce{[emim][B(CN)_4]} than in \ce{[emim][Tf_2N]}.
...
By using TRAVIS \cite{Brehm_2011}, we have obtained SDFs of both cation and anion around a \ce{CO_2} molecule, sampled from MD trajectories of either \ce{CO_2}+\ce{[emim][B(CN)_4]} or \ce{CO_2}+\ce{[emim][Tf_2N]} systems simulated at $T = 298.0~\text{K}$, $p = 1.0~\mathrm{bar}$, and $x_\mathrm{CO_2} = 0.1$.
...
In addition, the first solvation shell corresponding to \ce{[B(CN)_4]^-} occurs at a slightly shorter distance than that corresponding to \ce{[Tf_2N]^-}, as one can clearly see by a direct comparison done in Fig.~\ref{fig:sdf_ions}(c).
...
This seems to induce a greater stability of \ce{CO_2} when solvated in \ce{[emim][B(CN)_4]}, rather than in \ce{[emim][Tf_2N]}, possibly resulting in enhanced solubility.
...
\caption{Spatial distribution functions (SDFs) of (a) \ce{[B(CN)_4]^-} (green) and \ce{[emim]^+} (yellow) at the isosurface values of 0.138 and 0.12, respectively; and of (b) \ce{[Tf_2N]^-} (blue) and \ce{[emim]^+} (white) at the isosurface values of 0.086 and 0.063, respectively.
Fig. (c) corresponds to the SDFs of the anions plotted together.}
\label{fig:sdf_ions}
\end{figure*}

\subsection{Concentrated solutions of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]}}
...
It is worth noting that the marked difference in the molar fractions of \ce{CO_2} in Table~\ref{table:solv} does not imply that \ce{[emim][Tf_2N]} is a better solvent than \ce{[emim][B(CN)_4]}, thus exemplifying the observation made by Carvalho \textit{et al}. \cite{Carvalho_2016} and discussed here in Sec.~\ref{sec:intro}.

\begin{table}
	\centering
	\caption{Thermodynamic conditions, based on the experimental observations of Refs.~\citenum{Makino_2014} and \citenum{Schilderman_2007}, employed in the solvation study at high concentrations of \ce{CO_2} in \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]}, as well as the results obtained via expanded ensemble simulations.}
	\begin{tabular}{ccc}
		\hline\hline
		IL & \ce{[emim][B(CN)_4]}  & \ce{[emim][Tf_2N]} \\
		\hline
%		model & Ref.~\citenum{Koller_2012} & Ref.~\citenum{Koddermann_2007} \\
		...
		$f_g$ & $16.9~\mathrm{atm}$ & $32.6~\mathrm{atm}$ \\
...
We first carried out simulations of liquid solutions at the conditions of Table~\ref{table:solv}. The model of Koller \textit{et al}. \cite{Koller_2012} was the one employed for simulating \ce{[emim][B(CN)_4]}. The computed solvation free energies for the systems \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][Tf_2N]} were $-0.755 \pm 0.009 ~\mathrm{kcal/mol}$ and $-0.542 \pm 0.005~\mathrm{kcal/mol}$, respectively. Using these values as inputs in Eq.~\eqref{eq:equilibrium pressure}, we obtained equilibrium pressures of p = 18.3 atm and 38.4 atm, as reported in Table~\ref{table:solv}.
In this table, we also show the the corresponding gas-phase fugacities of \ce{CO_2} ($f_s^g$) computed by Eq.~\eqref{eq:fgas_d}.
The results make clear that assuming ideal gas ($p = f_s^g$) would have been an oversimplification, specially in the case of \ce{[emim][Tf_2N]}.

In order to obtain $\Delta G_\text{sim} (p)$ and $c_s(p)$, we carried out extra simulations at pressure values  of p = (17, 18) atm  for \ce{CO_2}+\ce{[emim][B(CN)_4]} and at p = (40, 44) atm for \ce{CO_2}+\ce{[emim][Tf_2N]}.
In this case, we considered a wider range of pressure, given the departure from ideal gas behavior mentioned above.
As described in Sec.~\ref{sec:partial_pressure}, $\Delta G_\text{sim}$ and $c_s$ were interpolated to intermediate pressures by reweighting.
In Fig.~\ref{fig:pressure}, we show for both systems the simulated results of $\Delta G_\text{sim}$, together with the curve of $\Delta G_\text{sim}$ defined by Eq.~\eqref{eq:equilibrium pressure}.
As expected, $\Delta G_\text{sim}$ does not vary appreciably with pressure, at least over the range of experimental data considered here.
This results in small corrections to the equilibrium pressures of Table~\ref{table:solv}: the points of intersection are p = 17.8 atm and p = 40.5 atm, for \ce{CO_2}+\ce{[emim][B(CN)_4]} and \ce{CO_2}+\ce{[emim][Tf_2N]}, respectively.
These values did not depart considerably from the specified values, meaning that our results sustain the experimental observations.
Therefore, as the pressure required to reach the same equilibrium molal concentration of \ce{CO_2} in the liquid phase is smaller for \ce{[emim][B(CN)_4]} than for \ce{[emim][Tf_2N]}, our results confirm that the former is a better solvent than the latter.
...
Due to the availability of experimental data \cite{Yan_2010, Doma_ska_2011}, the solutes chosen for this study are benzene, hexane, cyclohexane, ethanol, and water. Uncertainties in activity coefficients were calculated using error propagation formula, as done for $K_s$.
...
In Fig.~\ref{fig:deltag_solutes}, we show the simulated solvation free energies versus the experimental results. These were obtained by using $\Delta G^{0}_\text{exp}$ of Table~\ref{table:mu_solutes} and $\gamma^\infty$  from Ref. \cite{Doma_ska_2011} as inputs in Eq.~\eqref{eq:gamma}. Cyclohexane was not considered, given that $\Delta G^{0}_\text{exp}$ is not available.
...
From the individual contributions of $\Delta G_\text{vdW}$ and $\Delta G_\text{Coul}$, we can conclude that the vdW contribution is largely underestimated. This makes sense since one would expect a less favorable insertion for a longer chain molecule in comparison, for instance, to benzene which has a comparable 
$\Delta G_\text{vdW}$.
...
%\begin{figure}
%	\includegraphics[width=\linewidth]{free_energy_solutes}
%	\caption{Profile of $\Delta G_\text{vdW}$ corresponding to the insertion of a \ce{CO_2} molecule in \ce{[emim][B(CN)_4]}, modeled according to the force fields of Koller \textit{et al}. \cite{Koller_2012} ($\bullet$), Batista \textit{et al}. \cite{Batista_2015} ($\blacksquare$), Liu \textit{et al}. \cite{Liu_2014} ($\times$), and Weber and Kirchner \cite{Weber_2016} ($\blacktriangle$).}
%	\label{fig:deltag}
%\end{figure}
...
\begin{figure}
	...
	\caption{Profile of $\Delta G_\text{vdW}$ corresponding to the insertion of a \ce{CO_2} molecule in \ce{[emim][B(CN)_4]}, modeled according to the.}
...
A key factor for its success seems to be the reparametrization of the LJ parameters for the boron atom carried out in the original work, which also allows to accurately reproduce the experimental density of the pure substance.

We focused also on exploring alternative simulation strategies with the goal of gaining computational efficiency.
Particularly, for both \ce{[emim][B(CN)_4]} and \ce{[emim][Tf_2N]}, we concluded that the Damped-Shifted-Force method for electrostatic interactions, with a damping factor $\alpha = 0.2 ~\text{\AA}^{-1}$, is a suitable strategy.
...
The solvation free energy results suggest that a more favorable solvation of \ce{CO_2} occurs in \ce{[emim][B(CN)_4]} than in \ce{[emim][Tf_2N]}, despite the similarity in the Henry's law constants observed in both cases.
This picture is further confirmed at high \ce{CO_2} concentrations, in which the simulations correctly predicted that a considerably lower pressure is required to dissolve a certain amount of \ce{CO_2} in \ce{[emim][B(CN)_4]}, as compared to \ce{[emim][Tf_2N]} in an equal-mass basis.
